...
var getTemplates = function (skin, input) {
  ...
  if (input) {
    ...
    configure = require(path.join(input + '/xzdoc.json'));
  } else {
    ...
    configure = require(path.join(__dirname, '../templates/' + skin + '/xzdoc.json'));
  }
...
exports.process = function (input, output, skin, dir) {
  ...
  if (fs.existsSync(path.resolve(input, 'xzdoc-templates/' + skin))) {
    themeDir = path.resolve(input, 'xzdoc-templates/' + skin);
...
exports.export = function (skin, callback) {
  var outputTheme = path.resolve('./xzdoc-templates/');
  ...
  ncp(path.join(__dirname, '../templates/' + skin + '/'), path.join('./xzdoc-templates/', skin), callback);
};
